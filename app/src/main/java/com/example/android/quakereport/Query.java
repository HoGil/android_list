...
public class Query {
    ...
    // create a private constructor because no one should create an object
    ...
    // return a list of eq objects that has been built up from parsing the json response
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray featuresArray = rootJson.getJSONArray("features");
            // get the array
            for (int i = 0; i < featuresArray.length(); i++) {
                JSONObject eqJsonObject = featuresArray.getJSONObject(i);
                JSONObject propObject = eqJsonObject.getJSONObject("properties");
                String mag = propObject.getString("mag");
                String location = propObject.getString("place");
                String time = propObject.getString("time");

                // create eq object
                Earthquake eqObject = new Earthquake(mag, location, time);
                earthquakes.add(eqObject);
            }


        } catch (JSONException e) {

            Log.e("Query", "Problem parsing eq json results.", e);
        }
